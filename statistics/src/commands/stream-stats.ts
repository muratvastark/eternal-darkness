...
const StreamStatistics: ICommand = {
    usages: ['strstat', 'strstats', 'streamstats', 'streamstat'],
    ...
    execute: async ({ client, message, args }) => {
        ...
        let count = 0;
        let total = 0;
        Object.keys(document.streams)
            .filter((d) => wantedDay > document.days - Number(d))
            .forEach((d) => {
                total += document.streams[d].total;
                count += document.streams[d].count;
                voice += document.streams[d].voice;
            });

        const emoji = formatEmoji(client.config.EMOJIS.STREAM_STAT);
        message.channel.send({
            embeds: [
                new EmbedBuilder({
                    color: client.utils.getRandomColor(),
                    description: [
                        `${bold('Streamer Verileri')}\n`,
                        `${user} adlı kullanıcının ${bold(`${wantedDay} günlük`)} ses istatistikleri;\n`,
                        `${emoji} Toplam Yayın Aktifliği: ${inlineCode(client.utils.numberToString(voice))}`,
                        `${emoji} Toplam Yayın Adeti: ${inlineCode(`${count} adet`)}`,
                        `${emoji} Toplam Yayın Süresi: ${inlineCode(client.utils.numberToString(total))}`,
                    ].join('\n'),
                }),
            ],
        });
